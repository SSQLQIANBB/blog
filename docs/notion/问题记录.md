...
2. 老鼠试毒（假设4瓶水）
    - 4 瓶水（1、2、3、4），1 瓶有毒。
    - 用 2 只小白鼠（A 和 B）就够了，原理如下：
        1. 给每瓶水编一个 “2 位二进制数”（因为 2 只老鼠能表示 2²=4 种情况）：
            - 1 号 → 01（二进制）
            - 2 号 → 10（二进制）
            - 3 号 → 11（二进制）
            - 4 号 → 00（二进制）
        2. 让老鼠按 “二进制位” 喝水：
            - 老鼠 A 负责看 “第一位”（左边那位）：如果某瓶水的第一位是 1，就让 A 喝（比如 2 号、3 号）；
            - 老鼠 B 负责看 “第二位”（右边那位）：如果某瓶水的第二位是 1，就让 B 喝（比如 1 号、3 号）。（简单说：3 号的二进制是 11，所以 A 和 B 都要喝 3 号的水）
        3. 看结果：
            - 假设 A 死了、B 死了 → 二进制 “11” → 对应 3 号有毒；
            - 假设 A 死了、B 活了 → 二进制 “10” → 对应 2 号有毒；
            - 假设 A 活了、B 死了 → 二进制 “01” → 对应 1 号有毒；
            - 假设 A 活了、B 活了 → 二进制 “00” → 对应 4 号有毒。
